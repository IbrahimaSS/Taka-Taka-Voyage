...
// Middleware qui attache l'utilisateur si possible, mais ne bloque pas si absent
exports.peuplerUtilisateur = async (req, res, next) => {
    try {
        const authHeader = req.headers.authorization;
        const cookieHeader = req.headers.cookie || "";
        ...
        if (token) {
            try {
                ...
                const utilisateur = await Utilisateur.findById(donnees.id).select("-motDePasse");
                if (utilisateur) {
                    req.utilisateur = utilisateur;
                }
            } catch (err) {
                // Token invalide, on ignore juste
            }
        }
        next();
    } catch (error) {
