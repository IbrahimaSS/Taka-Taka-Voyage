...
exports.statsPaiements = async (req, res) => {
    try {
        // Stats des paiements PAYE
        const paidStats = await Paiement.aggregate([
            {
                $match: { statut: "PAYE" },
            },
            {
                $group: {
                    ...
                    totalMontantChauffeur: { $sum: "$montantChauffeur" },
                },
            },
        ]);
        ...
        const enAttente = await Paiement.countDocuments({ statut: "EN_ATTENTE" });
        const annules = await Paiement.countDocuments({ statut: "ANNULE" });

        const result = paidStats[0] || {
            revenusTotaux: 0,
            totalPayes: 0,
            totalCommissions: 0,
            totalMontantChauffeur: 0,
        };
        ...
        return res.status(200).json({
            ...
            cards: {
                ...
                totalPaiements,
                totalPayes: result.totalPayes,
                totalCommissions: result.totalCommissions,
                totalMontantChauffeur: result.totalMontantChauffeur,
                enAttente,
                annules,
            },
        });
...
exports.repartitionRevenusParType = async (req, res) => {
    try {
        const data = await Paiement.aggregate([
            { $match: { statut: "PAYE" } },
            {
            ...
            {
                $group: {
                    _id: "$reservationDetails.typeVehicule",
                    totalMontant: { $sum: "$montantTotal" },
                    count: { $sum: 1 }
                }
            }
        ...
        const total = data.reduce((acc, curr) => acc + curr.totalMontant, 0);

        const repartition = data.map(item => ({
            type: item._id,
            montant: item.totalMontant,
            count: item.count,
            percentage: total > 0 ? ((item.totalMontant / total) * 100).toFixed(1) : 0
        }));
        ...
        console.error(error);
        return res.status(500).json({ succes: false, message: "Erreur lors de la récupération de la répartition par type" });
    }
};
...
exports.listePaiements = async (req, res) => {
    try {
        ...
        // Filtre par méthode
        if (req.query.methode && req.query.methode !== 'all') {
            filter.methode = req.query.methode;
        }
        ...
        const paiements = await Paiement.find(filter)
            .populate({
                path: "reservation",
                populate: [
                    { path: "passager", select: "nom prenom telephone email noteMoyenne photoUrl" },
                    { path: "chauffeur", select: "nom prenom telephone email noteMoyenne vehicule photoUrl" },
                ],
                select: "depart destination distanceKm dureeMin typeVehicule statut paiement createdAt",
            })
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit);
