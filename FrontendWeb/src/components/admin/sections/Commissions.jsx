...
const Commissions = () => {
  ...
  const exportColumns = useMemo(() => [
    ...
    { header: 'Chauffeur', accessor: 'nom' },
    { header: 'Email', accessor: 'email' },
    { header: 'Téléphone', accessor: 'telephone' },
    {
      header: 'Service', accessor: 'service', formatter: (value) => {
        const labels = { MOTO: 'Moto-taxi', TAXI: 'Taxi', VOITURE: 'Voiture privée', BUS: 'Bus' };
        return labels[value] || value;
      }
    },
    { header: 'Revenus (GNF)', accessor: 'montantBrut', formatter: (value) => value?.toLocaleString('fr-FR') },
    { header: 'Commission (GNF)', accessor: 'commission', formatter: (value) => value?.toLocaleString('fr-FR') },
    { header: 'À verser (GNF)', accessor: 'montantNet', formatter: (value) => value?.toLocaleString('fr-FR') },
    {
      header: 'Statut', accessor: 'statut', formatter: (value) => {
        return value === 'PAYE' ? 'Payé' : 'À payer';
      }
    },
    {
      header: 'Méthode', accessor: 'methode', formatter: (value) => {
        const labels = { ORANGE_MONEY: 'Orange Money', MTN_MONEY: 'MTN Money', CASH: 'Espèces' };
        return labels[value] || value;
      }
    },
    { header: 'Date Création', accessor: 'createdAt', formatter: (value) => value ? new Date(value).toLocaleDateString('fr-FR') : 'N/A' },
  ], []);
  ...
  const stats = useMemo(() => [
    {
      ...
      percentage: 0,
      progress: 75,
      subtitle: 'Total des commissions du mois (GNF)'
    },
    {
      title: 'À verser',
      ...
      subtitle: 'Montant total en attente (GNF)'
    },
    {
      ...
      percentage: 0,
      progress: 50,
      subtitle: 'Chauffeurs payés ce mois'
    }
  ], [statsCards]);
  ...
  const handleProcessPayment = async (paymentId, comment = '') => {
    ...
    try {
      ...
      if (res.data?.succes) {
        showToast('Paiement traité', 'Le paiement a été marqué comme versé', 'success');
        ...
        showToast('Erreur', res.data?.message || 'Erreur lors du traitement', 'error');
        setModalState(prev => ({ ...prev, loading: false }));
      }
    } catch (err) {
      console.error(err);
      showToast('Erreur', 'Impossible de traiter le paiement', 'error');
      setModalState(prev => ({ ...prev, loading: false }));
    }
  };
  ...
  const handleBulkProcess = async () => {
    ...
    try {
      ...
      console.error(err);
      showToast('Erreur', 'Erreur lors du traitement en masse', 'error');
      setModalState(prev => ({ ...prev, loading: false }));
    }
  };
  ...
  const handleEditPayment = async () => {
    ...
    try {
      ...
      if (res.data?.succes) {
        showToast('Paiement modifié', 'Les informations ont été mises à jour', 'success');
        setModalState(prev => ({ ...prev, loading: false, showEdit: false }));
        fetchList();
      } else {
        showToast('Erreur', res.data?.message || 'Erreur lors de la modification', 'error');
        setModalState(prev => ({ ...prev, loading: false }));
      }
    } catch (err) {
      console.error(err);
      showToast('Erreur', 'Impossible de modifier le paiement', 'error');
      setModalState(prev => ({ ...prev, loading: false }));
    }
  };
  ...
  const renderService = (service) => {
    const config = {
      MOTO: { label: 'Moto-taxi', color: 'green', icon: Car },
      TAXI: { label: 'Taxi', color: 'blue', icon: Car },
      VOITURE: { label: 'Voiture privée', color: 'purple', icon: Car },
      BUS: { label: 'Bus', color: 'orange', icon: Car }
    };
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mt-4 mb-3">Informations de paiement</h3>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-slate-200/30 dark:bg-gray-800 rounded-xl p-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="text-center">
                          <p className="text-sm text-gray-500 dark:text-gray-400">Montant brut</p>
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-900/40 rounded-xl p-4 space-y-3">
                      ...
                      {meta.verseLe && (
                        <div className="flex justify-between">
                          <span className="text-gray-600 dark:text-gray-300">Date de versement :</span>
    ...
    return (
      ...
      >
        <div className="space-y-6 scroll-m-t-2 overflow-y-auto h-[70vh]">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Méthode de paiement</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Numéro de compte / téléphone</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Commentaire (optionnel)</label>
              <textarea
          ...
          <div className="bg-blue-50 rounded-xl p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-300">À verser:</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2">
          <ChartCard
            ...
            subtitle="Données réelles de la plateforme"
            chartConfig={{
              type: 'line',
              data: {
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {repartitionData.length > 0 ? repartitionData.map((item, idx) => {
              const serviceLabels = { MOTO: 'Moto-taxi', TAXI: 'Taxi', VOITURE: 'Voiture privée', BUS: 'Bus' };
              ...
              <div className="text-center py-4">
                <p className="text-gray-500 dark:text-gray-400 text-sm">Aucune donnée de répartition disponible</p>
              </div>
            )}
      ...
      <Card hoverable={false}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="relative sm:col-span-2">
            ...
            <input
              type="text"
              placeholder="Rechercher un chauffeur par nom ou téléphone..."
            ...
            <div>
              <CardTitle>
                {activeTab === 'pending' ? 'Paiements en attente' :
                  activeTab === 'paid' ? 'Paiements payés' : 'Tous les paiements'}
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                ...
                {payments.length} affiché(s) sur {totalItems}
              </p>
            </div>
