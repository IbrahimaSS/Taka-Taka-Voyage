...
const Profile = () => {
  ...
  const renderAvatar = () => {
    ...
    const getImageUrl = (avatar) => {
      if (!avatar) return <User className="w-16 h-16 text-gray-400" />;
      if (avatar.startsWith("data:")) return avatar;
      if (avatar.startsWith("/uploads")) return `${API_URL}${avatar.startsWith('/') ? avatar.substring(1) : avatar}`;
      if (avatar.startsWith("http")) return avatar;
      return "/public/acceuil.jpg";
    };
            ...
            <div className="relative">
              ...
              {isEditing && (
                <button
                  ...
                  className="absolute bottom-2 right-2 w-10 h-10 bg-green-600 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-green-700 transition-colors animate-in zoom-in duration-200"
