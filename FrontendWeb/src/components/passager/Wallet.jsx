...
const Wallet = () => {
  ...
  useEffect(() => {
    ...
    const handleUpdate = (data) => {
      console.log("🔔 Socket Event Received (Passenger):", data);
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-center">
        ...
        >
          ...
          <div className="flex justify-between items-start z-10">
            <div>
              <p className="text-white/70 text-[9px] font-bold tracking-[0.2em] uppercase">PORTEFEUILLE PASSAGER</p>
          ...
          <div className="z-10 flex justify-between items-end bg-black/5 -mx-6 -mb-6 px-6 py-3 backdrop-blur-sm border-t border-white/5">
            <div>
              ...
              <p className="font-bold text-xs tracking-wide uppercase truncate max-w-[150px]">{passenger?.prenom} {passenger?.nom}</p>
            </div>
            <div className="text-right">
              <p className="text-[8px] uppercase opacity-60 font-bold mb-0.5">Contact</p>
              <p className="font-mono text-[10px] opacity-90">{passenger?.telephone || passenger?.phone}</p>
            </div>
          </div>
        </motion.div>
          ...
          {showModal.type === 'retrait' && (
            <div className={`bg-amber-50 dark:bg-amber-900/20 p-4 rounded-xl border border-amber-200 dark:border-amber-800/30 flex items-start space-x-3 ${otpStep ? 'hidden' : ''}`}>
              ...
              <p className="text-xs text-amber-800 dark:text-amber-300">
                Par sécurité, un code de validation vous sera envoyé par e-mail avant traitement.
              </p>
            </div>
          )}
            ...
            >
              <div className="text-center p-4 bg-emerald-50 dark:bg-emerald-950/20 rounded-2xl border border-emerald-100 dark:border-emerald-800/30">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                  Saisissez le code à 4 chiffres envoyé à votre adresse <b>{passenger?.email}</b>
                </p>
              </div>
