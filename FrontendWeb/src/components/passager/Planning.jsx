...
import {
  Calendar, Clock, MapPin, Car as CarIcon, Users,
  ...
  Motorbike, FileText, Printer, Phone
} from 'lucide-react';
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-3 pt-4">
            ...
            {(trip.status === 'EN_ATTENTE' || trip.status === 'ACCEPTEE') && (
              <Button variant="danger" onClick={() => onDelete(trip.id)} icon={Trash2} size="large" className="w-13 h-12 flex-shrink-0" />
            )}
          </div>
        </div>
      )}
