...
import {
  ...
  Shield, AlertCircle, Loader, CheckCircle2,
  Send, QrCode, Info, ChevronRight, Search
} from 'lucide-react';
  ...
  const handleAction = async (e) => {
    ...
    try {
      ...
      if (showModal.type === 'depot') {
        ...
        res = await PaymentService.retirer(amount, method, phone);
    ...
    <div className="max-w-6xl mx-auto p-4 space-y-8 pb-10">
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-center">
        ...
        <motion.div
          ...
          className="relative group h-44 w-full lg:w-[380px] shrink-0 overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-500 to-blue-600 text-white p-6 shadow-2xl flex flex-col justify-between border border-white/10"
        >
          ...
          <div className="flex justify-between items-start z-10">
            <div>
              <p className="text-white/70 text-[9px] font-bold tracking-[0.2em] uppercase">PORTEFEUILLE TAKA-TAKA</p>
          ...
          <div className="z-10 flex justify-between items-end bg-black/5 -mx-6 -mb-6 px-6 py-3 backdrop-blur-sm border-t border-white/5">
            <div>
              <p className="text-[8px] uppercase opacity-60 font-bold mb-0.5">Propriétaire</p>
              <p className="font-bold text-xs tracking-wide uppercase">{passenger?.prenom} {passenger?.nom}</p>
            </div>
            <div className="text-right">
              <p className="text-[8px] uppercase opacity-60 font-bold mb-0.5">Numéro de téléphone</p>
              <p className="font-mono text-[10px] opacity-90">{passenger?.telephone}</p>
            </div>
          </div>
        </motion.div>
          ...
          {actions.map((action, idx) => (
            ...
            >
              ...
              <div className="text-center">
                <span className="block font-black text-gray-900 dark:text-white text-sm leading-tight">{action.name}</span>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-emerald-50 dark:from-gray-800 dark:to-emerald-950/20 p-6 rounded-3xl border border-blue-100 dark:border-blue-900/30 flex items-start space-x-4">
        ...
        <div>
          <h4 className="font-bold text-gray-900 dark:text-white">Votre argent est sécurisé</h4>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            TakaTaka utilise un système de séquestre bancaire. En cas de litige ou d'annulation, vos fonds sont automatiquement protégés et reversés sur votre solde.
          </p>
        </div>
      </div>
      ...
      >
        <form onSubmit={handleAction} className="space-y-6">
          {showModal.type === 'retrait' && (
            <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-xl border border-amber-200 dark:border-amber-800/30 flex items-start space-x-3">
              <Info className="text-amber-600 mt-0.5 shrink-0" size={18} />
              <p className="text-xs text-amber-800 dark:text-amber-300">
                Les demandes de retrait sont validées par notre équipe sous 2h à 24h pour garantir la sécurité de votre argent.
              </p>
            </div>
          )}
          ...
          {(showModal.type === 'retrait' || showModal.type === 'transfert') && (
            <div>
              ...
              <input
                type="text"
                placeholder={showModal.type === 'transfert' ? "Bénéficiaire TakaTaka" : "Virement vers..."}
              ...
              />
              <p className="text-[10px] text-gray-500 mt-1 px-1 italic">Vérifiez bien le numéro avant de valider.</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              <div className="flex flex-col items-center">
                <div className="w-10 h-10 mb-2 flex items-center justify-center rounded-lg overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800 bg-white">
                  <img
                ...
                <span className="text-sm font-bold text-center leading-tight dark:text-white">Orange Money</span>
                <span className="text-[10px] text-gray-500 dark:text-gray-400 line-clamp-1 mt-1">Paiement instantané</span>
              </div>
            </button>
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <span className="text-sm font-bold text-center leading-tight dark:text-white">MTN/Areeba</span>
                <span className="text-[10px] text-gray-500 dark:text-gray-400 line-clamp-1 mt-1">Paiement instantané</span>
              </div>
            </button>
          </div>
          ...
          <Button
            ...
            size="large"
            type="submit"
            className="h-14 font-black text-lg shadow-xl shadow-green-500/20"
          >
