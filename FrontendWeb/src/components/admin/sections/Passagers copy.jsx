...
const Users = ({ showToast }) => {
  ...
  useEffect(() => {
    fetchUsers();
    fetchStats();
  }, [currentPage, pageSize, searchTerm, selectedFilters]);
  ...
  const exportColumns = useMemo(() => [
    ...
    { header: 'Trajets', accessor: (u) => u.statistiques?.trajetsEffectues || 0 },
    { header: 'Note', accessor: (u) => u.note || '-' },
  ...
  const stats = [
    {
      ...
      icon: UserCheck,
      color: 'green',
      trend: statsData?.evolution >= 0 ? 'up' : 'down',
      percentage: Math.abs(statsData?.evolution || 0),
      progress: 75,
      description: 'Total inscrits'
    },
    {
      title: 'Nouveaux ce mois',
      value: statsData?.nouveauxMois?.toLocaleString() || '0',
      icon: Activity,
      color: 'blue',
      trend: 'up',
      percentage: 0,
    ...
    },
    {
      ...
      value: statsData?.actifsAujourdhui?.toLocaleString() || '0',
      icon: Star,
      color: 'purple',
      trend: 'stable',
      percentage: 0,
          ...
          <Table headers={['Utilisateur', 'Trajets', 'Inscription', 'Statut', 'Actions']}>
            {users.map((user) => (
              <TableRow key={user._id}>
                ...
                <TableCell>
                  <div className="font-bold text-gray-800 dark:text-gray-100">{user.statistiques?.trajetsEffectues || 0}</div>
                </TableCell>
                <TableCell>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:bg-gray-900 rounded-xl p-4 text-center">
                <div className="text-2xl font-bold text-gray-800 dark:text-gray-900">{selectedUser.statistiques?.trajetsEffectues || 0}</div>
              ...
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:bg-gray-900 rounded-xl p-4 text-center">
                <div className="flex items-center justify-center">
                  ...
                  <span className="text-2xl font-bold text-gray-800 dark:text-gray-900">{selectedUser.note || '-'}</span>
                </div>
                <div className="text-sm text-gray-500 dark:text-gray-900">Note moyenne</div>
              </div>
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-4 text-center">
                {/*  TODO: Recupérer le total dépensé depuis le backend si dispo */}
                <div className="text-2xl font-bold text-gray-800 dark:text-gray-900">
                  {selectedUser.totalSpent ? `${selectedUser.totalSpent.toLocaleString()} GNF` : '-'}
                </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{selectedUser.localisation?.adresse || 'Non renseignée'}</span>
                  </div>
                </div>
              </div>
            </div>
