...
const Sidebar = ({
  ...
  const menuItems = useMemo(() => {
    return config.menuItems.map(item => {
      const mainPath = item.path === '.' ? '' : item.path;
      let subItems = item.subItems?.map(sub => ({
        ...sub,
        count: navCounts[`${mainPath}_${sub.path}`] !== undefined ? navCounts[`${mainPath}_${sub.path}`] : undefined
      }));
      return {
        ...item,
        count: navCounts[mainPath] !== undefined ? navCounts[mainPath] : item.count,
        subItems
      };
    });
  }, [config.menuItems, navCounts]);
